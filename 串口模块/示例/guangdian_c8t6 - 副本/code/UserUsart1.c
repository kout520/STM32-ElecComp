...
void user_uart_process(void) {
    if(usart1_recvFrame == 1) {
        // 最小有效数据检查：@(x.x)\r\n 至少需要7字节
        if(usart1_RecvLen >= 7) {
            // 检查帧头和帧尾
            if(usart1_RecvBuf[0] == '@' && 
               usart1_RecvBuf[1] == '(' && 
               usart1_RecvBuf[usart1_RecvLen-2] == '\r' && 
               usart1_RecvBuf[usart1_RecvLen-1] == '\n') {
                ...
                // 查找右括号位置
                uint8_t *right_brace = memchr(usart1_RecvBuf, ')', usart1_RecvLen);
                if(right_brace) {
                    // 计算数字部分长度
                    num_len = right_brace - (usart1_RecvBuf + 2);
                    if(num_len > 0 && num_len < sizeof(num_str)) {
                        memcpy(num_str, usart1_RecvBuf + 2, num_len);
                        num_str[num_len] = '\0';
                        
                        // 转换为浮点数
                        float yaw_value = 0.0f;
                        if(sscanf((char*)num_str, "%f", &yaw_value) == 1) {
                            // 解析成功，这里可以添加业务处理逻辑
                            printf("Yaw parsed: %.2f\n", yaw_value);
                            
                            // 示例：通过CAN发送（需要实现CAN发送函数）
                            // CAN1_SendYaw(yaw_value);
                        }
                    }
                }
            }
            ...
            // 调试用：回传原始数据（可选）
            Usart1_SendData(usart1_RecvBuf, usart1_RecvLen);
        }
        
        usart1_recvFrame = 0;
        Usart1_ReadData(); // 重启接收
    }
}
