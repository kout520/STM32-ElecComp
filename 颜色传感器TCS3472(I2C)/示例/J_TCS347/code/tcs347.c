...
void tcs_app(void) {
    ...
    TCS34725_GetRawData(&rgb);  
    RGBto255RGB(&rgb, &rgb_255A);
    printf("R255=%d G255=%d B255=%d Lux=%d \r\n", rgb_255A.r, rgb_255A.g, rgb_255A.b, rgb_255A.Lux);

    // 定义不同光照强度下的颜色阈值表
    static const ColorThreshold thresholds[] = {
        // Lux < 800 的阈值
        {175, 190, 245, 255, 164, 180, 1},  // 条件1‘黄色’
        {125, 140, 245, 255, 170, 180, 2},  // 条件2‘绿色’
        {154, 165, 245, 255, 245, 255, 3},  // 条件3‘蓝色’
        {230, 255, 245, 255, 220, 255, 4},  // 条件4‘红色’
        {140, 152, 245, 255, 205, 220, 5},  // 条件5‘深绿’
        {245, 255, 208, 225, 210, 230, 6},  // 条件6‘粉色’
        {190, 205, 245, 255, 245, 255, 7},  // 条件7‘紫色’
        {145, 154, 245, 255, 245, 255, 8},  // 条件8‘浅蓝’
        {245, 255, 245, 255, 180, 195, 9},  // 条件9‘橙色’


        // Lux 3500-4500 的阈值（根据实际需求调整）
        {175, 190, 245, 255, 164, 180, 1},  // 条件1‘黄色’
        {125, 140, 245, 255, 170, 180, 2},  // 条件2‘绿色’
        {154, 165, 245, 255, 245, 255, 3},  // 条件3‘蓝色’
        {230, 255, 245, 255, 220, 255, 4},  // 条件4‘红色’
    ...
    // 根据光照强度选择阈值范围
    size_t start_idx = 0;
    size_t end_idx = 8;  // Lux < 800 的5个条件
    if (rgb_255A.Lux >= 800 && rgb_255A.Lux <= 4500) {
        start_idx = 9;   // Lux 3500-4500 的5个条件
        end_idx = 17;
    } else if (rgb_255A.Lux > 4500) {
        start_idx = 18;  // Lux > 4500 的5个条件
        end_idx = 26;
    }
    ...
    for (size_t i = start_idx; i <= end_idx; i++) {
        ...
        if (rgb_255A.r >= t->min_r && rgb_255A.r <= t->max_r &&
            rgb_255A.g >= t->min_g && rgb_255A.g <= t->max_g &&
            rgb_255A.b >= t->min_b && rgb_255A.b <= t->max_b) {
            tcs_flag = t->flag;
            break;  // 匹配到第一个符合条件的阈值即退出
        }
    }
    printf("%d\r\n",tcs_flag);
}
