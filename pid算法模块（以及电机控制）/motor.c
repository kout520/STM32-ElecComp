...
#define LEFT_MOTOR_PORT GPIOE

// 定义小车控制参数
#define BASE_SPEED_LEFT 65.0f      // 左轮基础速度 (m/s)
...
void Motor_Control_Init(void) {
    ...
    // 初始化 PID 控制器
    PID_Incremental_Init(&Speed_Pid_L, 1.0f, 0.0f, 0.05f, target_speed_L); // 左电机 PID
    // 初始化编码器
...
void Motor_Control_Update(void) {
    ...
    // 计算 PID 输出
     output_L = PID_Incremental_Calculate(&Speed_Pid_L, Encoder_Get_Speed(&Encoder_L));
    // 设置电机 PWM 占空比
...
void Differential_Control(float error) {
    ...
    // 更新 PID 目标速度
    PID_SetTarget(&Speed_Pid_L, target_speed_L);
    // 设置电机方向为正转
    Set_Motor_Direction(1);
}
/**
